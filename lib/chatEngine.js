...
// Intent classifiers beyond FAQ — these read off live data
const INTENTS = [
  {
...
function amenityIntent(q) {
  ...
  const withTerm = villas.filter((v) =>
    [...v.amenities, ...v.highlights].some((a) => a.toLowerCase().includes(term.split(/\s+/)[0]))
  );
  if (!withTerm.length) return null;
  return {
    reply: `Villas with ${term}: ${withTerm.map((v) => `**${v.name}**`).join(', ')}. Want details on one?`
  };
}

// Capacity lookup
function capacityIntent(q) {
...
function findVilla(q) {
  const lower = q.toLowerCase();
  return villas.find((v) => {
    const name = v.name.toLowerCase();
    // Match full name OR distinctive single word
 ...
 */
export function replyTo(userMessage) {
  ...
  const price = priceIntent(q);
  if (price) return price;

  const villa = villaIntent(q);
  if (villa) return villa;

  const cap = capacityIntent(q);
  if (cap) return cap;

  const loc = locationIntent(q);
  if (loc) return loc;

  const amenity = amenityIntent(q);
